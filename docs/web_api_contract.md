...
実装は [application/simulation_runner.py](c:/Users/galax/Desktop/3lvlpwm/application/simulation_runner.py) に集約した。

## 実装範囲

1. run_simulation(params)
2. build_web_response(results, max_points=1000)
3. normalize_ui_display_params(display_params, pwm_mode, fft_target, fft_window)
...
| R | Ω | 負荷抵抗 |
| L | H | 負荷インダクタンス |
| pwm_mode | - | natural / regular / third_harmonic |
| fft_target | - | voltage / current |
| fft_window | - | hann / rectangular |

desktop UI 側の補助単位は [application/simulation_service.py](c:/Users/galax/Desktop/3lvlpwm/application/simulation_service.py) の normalize_ui_display_params() で SI 単位へ変換してから渡す。
...
| キー | 内容 |
| --- | --- |
| meta | simulation_api_version, pwm_mode, sampling_mode, fft_target, fft_window |
| params | V_dc, V_ll_rms, f, f_c, t_d, V_on, R, L |
| time | 表示用時間軸 |
| reference | 生の三相参照波形 |
...
| metrics | m_a, m_f, THD, RMS, PF1, 理論/実測基本波 |

## Phase 2 の API エンドポイント

実装は [webapi/app.py](c:/Users/galax/Desktop/3lvlpwm/webapi/app.py) にある。
...
入力は [webapi/schemas.py](c:/Users/galax/Desktop/3lvlpwm/webapi/schemas.py) の SimulationRequest で検証する。
...
静的 UI は [webui/index.html](c:/Users/galax/Desktop/3lvlpwm/webui/index.html)、[webui/styles.css](c:/Users/galax/Desktop/3lvlpwm/webui/styles.css)、[webui/app.js](c:/Users/galax/Desktop/3lvlpwm/webui/app.js) にある。FastAPI は [webapi/app.py](c:/Users/galax/Desktop/3lvlpwm/webapi/app.py) から次を配信する。
...
この結果、[ui/visualizer.py](c:/Users/galax/Desktop/3lvlpwm/ui/visualizer.py) は widget 値の取得、描画、ファイル保存トリガーに集中し、シミュレーション利用ロジックは application 配下へ集約された。
...
| `requirements-web.txt` | numpy + fastapi + uvicorn のみ（matplotlib/scipy 除外） |
