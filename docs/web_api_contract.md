...
Phase 6 では、従来の pwm_mode / svpwm_mode という混在した表現を整理し、参照生成方式・サンプリング方式・クランプ方式の 3 軸で統一的に扱う。
...
3. normalize_ui_display_params(display_params, fft_target="voltage", fft_window="hann", overmod_view=False, reference_mode=None, sampling_mode=None, clamp_mode=None, pwm_mode=None, svpwm_mode=None)
...
| R | Ω | 負荷抵抗 |
| L | H | 負荷インダクタンス |
| reference_mode | - | sinusoidal / third_harmonic / minmax |
| sampling_mode | - | natural / regular |
| clamp_mode | - | continuous / dpwm1 / dpwm2 / dpwm3 |
...
後方互換として、入力時のみ次の旧フィールドも受理する。

| 旧項目 | 受理値 | 内部変換 |
| --- | --- | --- |
| pwm_mode | natural / regular / third_harmonic / svpwm / natural_overmod | reference_mode + sampling_mode へ正規化 |
| svpwm_mode | three_phase / dpwm1 / dpwm2 / dpwm3 / two_phase(alias) | clamp_mode へ正規化 |
...
| キー | 内容 |
| --- | --- |
| meta | simulation_api_version, reference_mode, sampling_mode, clamp_mode, modulation_summary_label, legacy_pwm_mode, overmod_view, fft_target, fft_window |
| params | V_dc, V_ll_rms, f, f_c, t_d, V_on, R, L, reference_mode, sampling_mode, clamp_mode, overmod_view |
| time | 表示用時間軸 |
| reference | 生の三相参照波形 |
...
旧クライアント互換のため、`pwm_mode` と `svpwm_mode` も引き続き入力として受理するが、応答メタデータは新しい 3 軸表現のみを返す。
