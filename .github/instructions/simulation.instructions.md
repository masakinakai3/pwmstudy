...
| モジュール | 関数シグネチャ | 出力 |
|---|---|---|
| `reference_generator.py` | `generate_reference(V_ll, f, V_dc, t, mode="sinusoidal")` | `(v_u, v_v, v_w)` |
...
- PWM 方式: `pwm_mode`、参照生成方式: `reference_mode`、サンプリング方式: `sampling_mode`
- 周波数: `f`, `f_c`（キャリア）
- 時間: `t`, 時間刻み: `dt`
...
- 三次高調波注入では零相成分を許容し、線間参照差の不変性を検証する
- 変調信号の値域: `[-1, 1]`
