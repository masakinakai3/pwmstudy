...
### 4.4 厳密離散時間更新（唯一の例外: for ループの使用）
- RL負荷ソルバーの時間ステップ更新は for ループを許容する
- ステップ間に依存関係があるため、時間方向の完全ベクトル化は困難
- ZOH（零次ホールド）仮定: PWM電圧は各ステップ内で一定、解析解ベースの更新式を使用する
- 係数計算は `expm1` を用い、極小抵抗条件でも桁落ちを避ける
