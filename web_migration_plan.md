...
等間隔ダウンサンプリングは、指令波形や電流のように比較的なめらかな信号を主対象とする。スイッチングパターン、キャリア、急峻な立ち上がりを含む電圧波形では、エッジ欠落を避けるため min/max ペア圧縮を優先する。
...
バックエンドの LRU キャッシュは、正規化済みリクエスト JSON のハッシュをキーにして同一条件の再計算を避ける想定である。キャッシュ対象は表示用レスポンスのみとし、アプリ更新や計算条件仕様の変更時は無効化する。
