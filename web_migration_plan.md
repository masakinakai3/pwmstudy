...
バックエンドの LRU キャッシュは、正規化済みリクエスト JSON と simulation ロジックのバージョン文字列を組み合わせたハッシュをキーにして同一条件の再計算を避ける想定である。キャッシュ対象は表示用レスポンスのみとし、最大 32 件を上限とする。simulation ロジック更新時はバージョン文字列を更新して無効化する。
