...
MVP の入力バリデーション初期値は、現行 UI と合わせて次を基準とする。
...
等間隔ダウンサンプリングは、指令波形と相電流のように比較的なめらかな信号へ使う。min/max ペア圧縮は、スイッチングパターン、キャリア、PWM による急峻な立ち上がりを含む相電圧・線間電圧へ使う。

ここでいう 1000 点上限は、各信号配列ごとに適用する。たとえば `reference.u`、`voltages.v_uv`、`currents.i_u` は、それぞれ独立に最大 1000 点まで返す。
...
バックエンドの LRU キャッシュは、正規化済みリクエスト JSON と simulation ロジックのバージョン文字列を組み合わせたハッシュをキーにして同一条件の再計算を避ける想定である。キャッシュ対象は表示用レスポンスのみとし、最大 32 件を上限とする。バージョン文字列は `SIMULATION_API_VERSION` のようなアプリ定数として管理し、simulation ロジックやレスポンス仕様が変わるたびに更新して無効化する。
