...
| B. Pyodide/WASM でブラウザ実行 | Python をブラウザで動かす | サーバ負荷が小さい | NumPy/描画/配布構成が複雑 |
...
配列長が大きいため、将来的には表示点間引きや圧縮も検討する。

## 8. 主なリスクと対策

| リスク | 内容 | 対策 |
| --- | --- | --- |
| 応答遅延 | スライダー変更ごとに全波形再計算が走る | デバウンス、表示点削減、キャッシュ |
| JSON サイズ肥大 | 時系列配列が長い | 表示用ダウンサンプリング、必要信号のみ返却 |
