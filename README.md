...
- **8パラメータスライダー + PWM方式選択**: Natural / Regular / Third Harmonic Injection / Space Vector PWM を切替可能
- **相変調方式切替**: 全PWM方式で 3相変調（連続）に加え、DPWM1/DPWM2/DPWM3 を選択可能
- **FFT表示切替**: 線間電圧 v_uv と相電流 i_u を切替表示、Hann / Rectangular 窓も選択可能
- **Web UI**: ブラウザから 8 パラメータ、PWM 方式、相変調方式、Overmod View、FFT 表示対象を変更可能
...
- **PWM方式比較**: Natural / Regular / Third Harmonic Injection / SVPWM の方式差を比較可能
...
58 件のテスト（物理妥当性検証 + application 層 + API/UI 疎通）が実行されます。
...
│   └── app.js                   # Web UI ロジック
├── tests/
│   └── test_simulation.py       # 物理妥当性 + application/API/UI テスト（58 件）
